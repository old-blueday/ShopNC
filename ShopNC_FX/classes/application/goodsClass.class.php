...
/**
* FILE_NAME : goodsClass.class.php D:\root\shopnc6_jh\classes\application\goodsClass.class.php
* 商品分类应用类
*
...
* @package 
* @subpackage 
* @version Sat Jul 04 09:56:48 CST 2009
*/
class GoodsClassClass extends DataBase {
	/**
	 * 添加新的商品分类
	 *
	...
	function addGoodsClass($input_param){
		try {
			...
			$goods_class_array['class_menu']        = $input_param['txt_class_menu'];        //是否导航显示
			...
			$goods_class_array['shop_id']			= NC_SHOP_ID;							//子店id

			$insert_rs = $this->InsertRow($goods_class_array, 'goods_class', 'class_id');
			if ($insert_rs){
				return true;
			}
			else {
	...
	function delGoodsClass($input_param,$filed){
		try {
			$rs = $this->DelRow($input_param,"goods_class",$filed);
			if ($rs) {
				return true;
			}
			else {
	...
	function modifyGoodsClass($input_param,$modify_id,$param="all"){
		try {
			...
			$update_rs	= $this->UpdateRow($modify_id,$goods_class_array,"goods_class","class_id");
			...
			if($input_param['txt_modify_sub'] == 1) {
				...
				if(is_array($sub_array) and count($sub_array)>0) {
					foreach ($sub_array as $v) {
						$this->UpdateRow($v,array('goods_type_id'=>$goods_class_array['goods_type_id']),"goods_class","class_id");
					}
				}
			}
		}
	...
	function modifyMoreGoodsClass($input_param,$condition){
		try {
			...
			$update_rs = $this->UpdateRows('goods_class', $input_param, $condition_str);
		}
		catch (Exception $e) {
			return false;
		}
	...
	function getGoodsClassInfo($condition,$field = "*"){
		try {
			//得到条件语句
			$condition_str = $this->getCondition($condition);
			$goods_class_array = $this->GetTheRow($condition_str,'goods_class',$field);
	...
	function getGoodsClass($condition,$obj_page,$field = "*"){
		//得到条件语句
		$condition_str = $this->getCondition($condition);
		if($condition_str == '') $condition_str .= " and goods_class.shop_id=".NC_SHOP_ID;
		
		//得到会员基本表中符合要求的数据
		$goods_class_array = $this->GetList($condition_str,$obj_page,'goods_class',$field,0);
	...
	function getCondition($conditon_array){
		...
		if ($conditon_array['class_id'] != "") {
			$condition_sql .= " and goods_class.class_id=".$conditon_array['class_id']." and goods_class.shop_id=".NC_SHOP_ID;
		}
		if ($conditon_array['order_by_class_sort'] == 'yes') {
			$condition_sql .= "and goods_class.shop_id=".NC_SHOP_ID." order by goods_class.class_top_id asc,goods_class.class_sort asc,goods_class.class_id asc";
		}
		...
		if ($conditon_array['class_top_id'] != "") {
			$condition_sql .= " and class_top_id=".intval($conditon_array['class_top_id'])." and shop_id=".NC_SHOP_ID;
		}
		if ($conditon_array['class_state'] != "") {
			$condition_sql .= " and class_state='".$conditon_array['class_state']."' and shop_id=".NC_SHOP_ID;
		}
	...
	function createGoodsClassArray(){
		...
		$creatfile = BasePath."/share/".NC_SHOP_DIR."goods_class_show.php";
		...
		foreach ($class_array as $va) {
			if($va['class_top_id'] != 0) {
				$sub_class = $this->GetList('and shop_id='.NC_SHOP_ID.' and class_top_id='.$va['class_id'],'','goods_class','class_id',0);
	...
	function listClassDetail($string="&nbsp;&nbsp;&nbsp;&nbsp;"){
		...
		if (!is_file(BasePath."/share/".NC_SHOP_DIR."goods_class_show.php")  || strlen(trim(file_get_contents(BasePath."/share/".NC_SHOP_DIR."goods_class_show.php")))<25 ){
	...
	function ajaxClassUpdate($array) {
		...
		$update_rs = $this->UpdateRow($array['class_id'],$class_array,"goods_class",'class_id');
		if($update_rs) {
