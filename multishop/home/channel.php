...
class ShowChannel extends ChannelFrameWork {
	/**
	 * 频道对象
	 *
	 * @var obj
	...
	function _save(){
		if($this->_input['indextype'] == '0'){
			...
			if ($this->_input['getBack'] == '1'){//恢复
				@header('Location: index_drag.php?action=control');
			}
	 ...
	 * 固定模板频道展示
	 */
	function _templates_show($channel_id){
		if (intval($channel_id) > 0){
			$this->obj_html_channel->_make_channel_html(intval($channel_id));
		}else {
	...
	function _show(){
		if (intval($this->_input['id']) > 0){
			//获取频道信息
			$channer_array = $this->obj_channel->getChannelById($this->_input['id']);
			if ($channer_array['channel_type'] == '3') {
				$this->_templates_show($this->_input['id']);
			}
			//取频道风格和模块内容
			$this->_get_diy_style($this->_input['id']);
			//判断是否登录
			...
			$this->output('config_channel_html',$this->_configinfo['websit']['channel_html']);
	 ...
	 * @return 数组形式的返回内容
	 */
	function _get_diy_style($channel_id){
		//取频道信息
		$channel_array = $this->obj_channel->getChannelById($channel_id);
		if (empty($channel_array)){
			$this->redirectPath('error','../index.html',$this->_lang['langChannelIdNonlicet']);
		}
		...
		if(is_array($block_list)){
			...
			foreach($block_list as $k => $v){
				switch($v['module_type']){
					case 'adv':
						$block_list[$k]['html'] = 'channel_control_adv.html';
						break;
					case 'vote':
						$block_list[$k]['html'] = 'channel_control_vote.html';
						break;
					case 'product':
						$block_list[$k]['html'] = 'channel_control_product.html';
						break;
					case 'pclass':
						$block_list[$k]['html'] = 'channel_control_product_class.html';
						break;
					case 'shop':
						$block_list[$k]['html'] = 'channel_control_shop.html';
						break;
					case 'shopclass':
						$block_list[$k]['html'] = 'channel_control_shop_class.html';
						break;
					case 'customize':
						$block_list[$k]['html'] = 'channel_control_customize.html';
						break;
					case 'news':
						$block_list[$k]['html'] = 'channel_control_news.html';
						break;
				}
				//
				if($v['show'] == ''){
