 ...
 */
   ...
   /**
    ...
    {
        require_once(BasePath.'/classes/libraries/common.class.php');
    	if ( file_exists($filename) && ($fd = @fopen($filename, 'rb')) ) {
            ...
            while (!feof($fd)) {
                $contents .= fread($fd, 8192);
						...
						/**
			 * Important!!!!
			 * Modified By ShopNC! Apply to the GBK version.
			 */
			if($this->cache_charset == 'GBK'){
				/*判断文件是否是UTF-8的，如果是则转码*/

				/*回到文件开始部分,准备读取文件头判断编码*/
				if(rewind($fd)===false){
				...
				if($char1==chr(239) && $char2==chr(187) && $char3==chr(191)){	//有文件头的，直接判断文件头编码
//					if(function_exists('iconv')){
//						$contents = iconv("UTF-8", "GBK//IGNORE", $contents);
//					}else{
						$contents = Common::nc_change_charset($contents,'utf8_to_gbk');
//						$contents = mb_convert_encoding($contents, "GBK", "UTF-8");
//					}
				}elseif($char1==chr(255) && $char2==chr(254)){	//UNICODE LE
					echo "不支持从UNICODE LE转换到UTF-8编码<br>";
					fclose($fd);
					return;
				}elseif($char1==chr(254) && $char2==chr(255)){	//UNICODE BE
					echo "不支持从UNICODE BE转换到UTF-8编码<br>";
					fclose($fd);
					return;
				}else{/*没有文件头,可能是GBK或UTF-8*/
