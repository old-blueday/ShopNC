...
class kqPay extends CommonFrameWork{
	...
	function main(){
		if($this->_configinfo['websit']['ncharset'] == 'UTF-8') {
			$this->inputCharset_99='1';
		} else {
			$this->inputCharset_99='2';
		}
		 ...
		 * 取当前预付款交易信息
		 */
		$record_array = $this->obj_predeposit->getOnePredepositRecordById(intval($this->_input['predeposit_r_id']));
		/**
		 * 验证信息是否与会员相符
		 */
		...
		$array['order_id'] = $record_array['predeposit_r_id'];    //订单号
		$array['order_amount'] = $record_array['online_amount']*100;    //金额
		$array['order_time'] = date('YmdHis',$record_array['create_time']);    //订单提交时间date('YmdHis');
		$array['product_name'] = $this->_lang['langPredepositPay'];    //商品名称
		$array['product_num'] = 1;
		$array['bg_url'] = $this->_configinfo['websit']['site_url']."/predeposit_pay/99bill/receive.php";    //返回地址
...
///默认值为1
$inputCharset=$obj_kqpay->inputCharset_99;
...
<html>
	<head>
		<title><?php echo $obj_kqpay->_lang['langPreCashTypeKqpay']; ?></title>
...
<BODY>
	...
	<div align="center">
		<table width="259" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC" >
			<tr bgcolor="#FFFFFF">
				<td width="80"><?php echo $obj_kqpay->_lang['langPrePayment']; ?>:</td>
				<td ><?php echo $obj_kqpay->_lang['langPreCashTypeKqpay']; ?></td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td ><?php echo $obj_kqpay->_lang['langPreDetailSpcode']; ?>:</td>
				<td ><?php echo $orderId; ?></td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td><?php echo $obj_kqpay->_lang['langPreDetailSpPrice']; ?>:</td>
				<td><?php echo $orderAmount/100; ?></td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td><?php echo $obj_kqpay->_lang['langPrePayName']; ?>:</td>
				<td><?php echo $payerName; ?></td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td><?php echo $obj_kqpay->_lang['langPrePayProductName']; ?>:</td>
	...
	<div align="center" style="font-size=12px;font-weight: bold;color=red;">
		<form name="99bill" method="post" action="https://www.99bill.com/gateway/recvMerchantInfoAction.htm">
			...
			<input type="submit" name="submit" value="<?php echo $obj_kqpay->_lang['langPrePaySubmit99']; ?>">
