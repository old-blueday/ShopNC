...
class feedcontrol extends base {
	...
	function onadd() {
		...
		$body_data	= $this->daddslashes(serialize($this->dstripslashes($this->input('body_data'))));
		$title_data = $this->daddslashes(serialize($this->dstripslashes($this->input('title_data'))));
		...
		$this->db->query("INSERT INTO ".substr(NC_DBTABLEPRE,0,-7)."home_feed SET appid='$appid', icon='$icon', uid='$uid', username='$username',
