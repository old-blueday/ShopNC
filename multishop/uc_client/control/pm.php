...
class pmcontrol extends base {
    ...
    function ondeleteids() {
        $this->init_input();
        $id = $_ENV['pm']->deletepmids($this->input('pmids'));
        return $id;
    }
    ...
    function onlsall() {
        ...
        $msglen = intval($this->input('msglen'));
        $result = array();

        $folder = in_array($folder,array('newbox','inbox','outbox','searchbox')) ? $folder : 'inbox';

        $pmnum = $_ENV['pm']->get_num_all($folder);
        $start = ($page-1) * $pagesize;

        if($pagesize > 0) {
            $pms = $_ENV['pm']->get_pm_list_all($pnum,$folder,$start,$pagesize);
