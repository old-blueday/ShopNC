...
class pmmodel {
	...
	function get_num_all($folder) {
        ...
        case 'outbox':
        case 'inbox':
            $filteradd = " related='0' AND folder='inbox' AND msgfromid='0'";
    ...
    function get_pm_list_all($pmnum,$folder,$start,$pagesize=20) {
        $filteradd = "pm.related='0' AND pm.folder='inbox' AND msgfromid='0'";
        $sql = "SELECT pm.*,m.username as msgtoname FROM ".UC_DBTABLEPRE."pms pm LEFT JOIN ".UC_DBTABLEPRE."members m ON pm.msgtoid=m.uid WHERE {$filteradd} ORDER BY pm.dateline DESC LIMIT $start,$pagesize";
    ...
    function deletepmids($pmids) {
        $this->db->query("DELETE FROM ".UC_DBTABLEPRE."pms WHERE pmid IN (".$this->base->implode($pmids).")");
