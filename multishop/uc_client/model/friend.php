...
class friendmodel {
	...
	function delete($uid, $friendids) {
		...
		$this->db->query("DELETE FROM ".NC_UCH_DBTABLEPRE."friend WHERE uid='$uid' AND fuid IN ($friendids)");
	...
	function get_totalnum_by_uid($uid, $direction = 0) {
		...
		$totalnum = $this->db->result_first("SELECT COUNT(*) FROM ".NC_UCH_DBTABLEPRE."friend WHERE $sqladd");
		return $totalnum;
	}
	...
	function get_list($uid, $page, $pagesize, $totalnum, $direction = 0) {
		...
		if($sqladd) {
			$data = $this->db->fetch_all("SELECT f.*, m.username FROM ".NC_UCH_DBTABLEPRE."friend f LEFT JOIN ".UC_DBTABLEPRE."members m ON f.fuid=m.uid WHERE $sqladd LIMIT $start, $pagesize");
			return $data;
		} else {
			return array();
		}
	...
	function is_friend($uid, $friendids, $direction = 0) {
		...
		if($direction == 0) {
			...
			$sqladd = "fuid='$uid' AND uid IN ('$friendid_str') AND direction='1'";
		} elseif($direction == 3) {
			$sqladd = "uid='$uid' AND friendid IN ('$friendid_str') AND direction='3'";
		}
		if($this->db->result_first("SELECT COUNT(*) FROM ".NC_UCH_DBTABLEPRE."friend WHERE $sqladd") == count($friendids)) {
			return true;
		} else {
			return false;
		}
