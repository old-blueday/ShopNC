...
!defined('IN_UC') && define('IN_UC', TRUE);
...
function uc_pm_deleteids($pmids) {
    return call_user_func(UC_API_FUNC,'pm','deleteids',array('pmids'=>$pmids));
}
...
function uc_pm_list_all($page,$pagesize=10,$folder='inbox',$msglen=100) {
    $page = intval($page);
    $pagesize = intval($pagesize);
    $msglen = intval($msglen);

    $return = call_user_func(UC_API_FUNC, 'pm','lsall',array('page'=>$page,'pagesize'=>$pagesize,'folder'=>$folder,'msglen'=>$msglen));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
 ...
 * ShopNc 专用方法
 *
 * 获取指定用户所加入的所有群组或指定群组所加入的所有的用户
 *
 * @param int $id 群组或用户id
 * @parma string $type $id类型 user/group
 * @return array
 */
function nc_uch_getgroup($id=0,$type='user',$isCount=false) {
    return call_user_func(UC_API_FUNC,'nc','getgroup',array('id'=>$id,'type'=>$type,'iscount'=>$isCount));
}

function nc_uch_getapp_by_type($type) {
    return call_user_func(UC_API_FUNC,'nc','getapp_by_type',array('type'=>$type));
}

function nc_uch_sharing($uid,$username,$type,$title_template,$body_template,$data,$body_general) {
    return call_user_func(UC_API_FUNC,'nc','sharing',array('uid'=>$uid,'username'=>$username,'type'=>$type,'title_template'=>$title_template,'body_template'=>$body_template,'data'=>$data,'body_general'=>$body_general));
}

/**
 * @param int $isone 如果$isone为true那么只获取指定$uid会员的动态信息
 */
function nc_uch_feed_get($uid,$friend,$limit,$isone=false) {
    return call_user_func(UC_API_FUNC,'nc','feed_get',array('uid'=>$uid,'friend'=>$friend,'limit'=>$limit,'isone'=>$isone));
}
?>