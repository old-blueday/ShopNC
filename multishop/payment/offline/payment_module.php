...
class offlinePaymentMethod extends CommonFrameWork{
	 ...
	 * 			$payment['field'] 会员扩展表数据库字段 线下不需要字段支持
	 * 			$payment['currency'] 该支付方式支持的货币种类
	 */
	function payment_param(){
		$payment['dir'] = 'offline';
		$payment['name'] = $this->_lang['langOOffline'];
		$payment['config'] = 'payment_offline';
		$payment['type'] = 'offline';
		$payment['field'] = 'offline';
		...
		switch ($param['order_state']){
			case '0'://已购买，初始状态,等待买家付款
				return array(
					...
					'state_url'=>$this->_configinfo['websit']['site_url'].'/member/own_order.php?action=offline_pay&sp_code='.$param['sp_code'],
					'state_type'=>'buyer',
				);
				break;
			case '1'://买家已付款,等待卖家发货
