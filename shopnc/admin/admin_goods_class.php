...
/**
* FILE_NAME : admin_goods_class.php \shopnc6\admin\admin_goods_class.php
* ....商品分类操作
*
...
* @package 
* @subpackage 
* @version Tue Jul 01 14:00:59 CST 2008
*/
require ("../global.inc.php");
class GoodsClassIndex extends SystemFrameWork {
	/**
	 * 商品对象
	 *
	 * @var obj
	...
	function main(){
		...
		if (!is_object($this->obj_goods_type)) {
			require_once("goodsType.class.php");
			$this->obj_goods_type = new GoodsTypeClass();
		}
		/**
		 * 创建语言包对象
		 */
		...
		if (!is_object($this->obj_log)){
			require_once("toolLog.class.php");
			$this->obj_log = new ToolLogClass();
		}
		/**
		 * 语言包
		 */
		$this->getlang("admin_goods,admin_menu");
		/**
		 * 设置模板路径
		 */
		$this->setsubtemplates("admin/templates");
		...
		switch($this->_input['action']){
			...
			$this->saveGoodsClass();
			break;
			case 'class_move':				//分类转移页面
			$this->moveClass();
			break;
			case 'move':					//转移分类
			$this->moveGoodsClass();
			break;
			case 'class_del':				//分类删除页面
			$this->delClass();
			break;
			case 'del':						//删除分类
			$this->delGoodsClass();
	...
	private function addGoodsClass() {
		...
		//是否发布在首页导航处
		$this->output("txt_class_menu",Common::Input("txt_class_menu","radio",array("1"=>$this->_lang['admin_goods_class_yes'],"0"=>$this->_lang['admin_goods_class_no']),$menu_value));
		//分类语言显示
		$language_array	= $this->obj_module_language->getLanguageList(array('language_state'=>1));
		if(is_array($language_array) and count($language_array)>0) {
			foreach ($language_array as  $v) {
				$language_select[$v['language_id']]	= $v['language_name'];
			}
		}
	...
	private function moveClass() {
		$goods_class_array	= array();
		$selectClassArray	= array();
		//得到商品分类信息
		$goods_class_array = $this->obj_goods_class->getGoodsClassInfo(array("class_id"=>intval($this->_input['cid'])));
		$this->output('goods_class_array',$goods_class_array);

		$selectClassArray = $this->obj_goods_class->listClassDetail();
		$productClass['class_top_id_2'] = Common::showForm_Select2("txt_class_top_id","","",$selectClassArray,"");
		$this->output('class_top_id_2',$productClass['class_top_id_2']);

		$this->showpage('goods_class_move');
	}
	/**
	 *  移动商品分类中的商品
	 *
	...
	private function moveGoodsClass(){
		...
		$class_top_id = $this->_input['txt_class_top_id'];
		if ($class_top_id == 0) {
		...
		if ($rs) {
			...
			$this->adminMessage($this->_lang['admin_goods_oper_fill'],$this->_configinfo['websit']['site_url']."/admin/admin_goods_class.php?action=page_goods_class",1,4000);
		}
	}
	/**
	 * 商品分类删除页面
	...
	private function delGoodsClass(){
		...
		$select_value = intval($this->_input['delclass']);
		$productClassArray = $this->obj_goods_class->listClassDetail("");
		switch ($select_value) {
	...
	private function updateClassAjax() {
		...
		if(!in_array($this->_input['ajax_type'],array('class_state','class_menu')) and intval($this->_input['class_id'])<= 0 ) {
			return '';
		}
...
$goods_class = new GoodsClassIndex();
$goods_class->main();
unset($goods_class);
?>